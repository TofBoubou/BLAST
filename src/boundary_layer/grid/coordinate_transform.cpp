...
auto compute_physical_y_from_eta(PhysicalQuantity auto eta, PhysicalQuantity auto xi, PhysicalQuantity auto rho_wall,
                                 ...
                                 PhysicalQuantity auto r_body) -> std::expected<double, TransformError> {
  ...
  auto rho_inv_range =
      rho_eta | std::views::take(eta_index + 1) | std::views::transform([](auto rho) { return 1.0 / rho; });
...
constexpr auto compute_derivative_factor(int station, PhysicalQuantity auto xi, const io::SimulationConfig& sim_config,
                                         PhysicalQuantity auto d_ue_dx, PhysicalQuantity auto rho_e,
                                         PhysicalQuantity auto mu_e, PhysicalQuantity auto u_e,
                                         PhysicalQuantity auto r_body) -> std::expected<double, TransformError> {
