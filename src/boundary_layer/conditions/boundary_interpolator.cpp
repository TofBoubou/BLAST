...
compute_property_derivatives(const std::vector<double> &values,
  ...
  // Use the high-order scheme only when the grid is uniform *and* sufficiently
  // populated. With very few points the high-order stencil can introduce
  // spurious oscillations which later propagate into the Hermite
  // interpolation, creating non-physical edge states. When the grid is
  // coarse, fall back to a simple slope-based derivative well suited for
  // adaptive refinement.

  if (is_uniform && values.size() >= 7) {
    // Reuse existing high-order O(h^4) function for well-resolved grids
    auto result =
  ...
  // Handle general case with weighted central differences (works for both
...
get_species_fractions(const io::OuterEdgeConfig::EdgePoint &edge_point,
  ...
  if (edge_point.boundary_override_enabled()) {
    if (!edge_point.mass_fraction_condition()) {
      return std::unexpected(
          BoundaryConditionError("mass_fraction_condition missing despite "
                                 "boundary_override being enabled"));
    }

    if (edge_point.mass_fraction_condition()->size() != mixture.n_species()) {
      return std::unexpected(BoundaryConditionError(std::format(
          "mass_fraction_condition size ({}) doesn't match number of species "
          "({})",
...
[[nodiscard]] auto
create_stagnation_conditions(const io::OuterEdgeConfig &edge_config,
  ...
  auto density =
      ...
      (edge_point.temperature * constants::physical::universal_gas_constant);
  ...
  EdgeConditions edge{.pressure = edge_point.pressure,
                      ...
                      .boundary_override =
                          edge_point.boundary_override_enabled()};
...
[[nodiscard]] auto
interpolate_boundary_conditions(int station, double xi,
  ...
  const auto x_grid = extract_edge_property(edge_config.edge_points,
  ...
  const auto u_edge = extract_edge_property(
      edge_config.edge_points, &io::OuterEdgeConfig::EdgePoint::velocity);
  const auto t_edge = extract_edge_property(
  ...
  const bool all_override = std::all_of(
      edge_config.edge_points.begin(), edge_config.edge_points.end(),
      [](const auto &p) { return p.boundary_override_enabled(); });
  const bool none_override = std::none_of(
      edge_config.edge_points.begin(), edge_config.edge_points.end(),
      [](const auto &p) { return p.boundary_override_enabled(); });
  ...
  std::vector<double> species_fractions;
  {
    ...
    if (!all_override && !none_override) {
      return std::unexpected(BoundaryConditionError(
          "All edge points must have the same boundary_override setting for "
          "interpolation"));
    }
    ...
    if (all_override) {
      ...
      if (n_species == 0) {
        return std::unexpected(
            BoundaryConditionError("Mixture reports zero species"));
      }
      ...
      for (std::size_t j = 0; j < n_species; ++j) {
        ...
        for (const auto &pt : edge_config.edge_points) {
          if (!pt.mass_fraction_condition()) {
            return std::unexpected(BoundaryConditionError(
                "Missing mass_fraction_condition in edge point with "
                "boundary_override enabled"));
          }
          const auto &y = *pt.mass_fraction_condition();
          if (y.size() != n_species) {
            return std::unexpected(BoundaryConditionError(std::format(
                "mass_fraction_condition size mismatch (got {}, expected {})",
                y.size(), n_species)));
          }
        ...
        species_fractions[j] =
            interpolate_property(yj_values, ix1, ix2, x_grid, x_interp);
        if (!std::isfinite(species_fractions[j])) {
          return std::unexpected(BoundaryConditionError(std::format(
              "Non-finite interpolated mass fraction for species {} at x = {}",
              j, x_interp)));
        }
      }

      // Clean small negatives (interp artifacts), enforce non-negativity and
      // renormalize
      for (auto &v : species_fractions) {
        ...
        if (v < 0.0) {
          return std::unexpected(
              BoundaryConditionError("Interpolated mass fractions contain "
                                     "negative values beyond tolerance"));
        }
      }
      double sum = std::accumulate(species_fractions.begin(),
                                   species_fractions.end(), 0.0);
      if (!(sum > 0.0) || !std::isfinite(sum)) {
        return std::unexpected(BoundaryConditionError(
            "Sum of interpolated mass fractions is not positive/finite"));
      }
      for (auto &v : species_fractions)
        v /= sum;
    } else {
      ...
      if (species_fractions.empty()) {
        return std::unexpected(BoundaryConditionError(
            "Equilibrium composition returned empty vector"));
      }
    }
  }
  ...
  const double density_interp =
      ...
      (temperature_interp * constants::physical::universal_gas_constant);
  ...
  if (!(d_xi_dx_calculated > 0.0) || !std::isfinite(d_xi_dx_calculated)) {
    return std::unexpected(BoundaryConditionError(
        std::format("Invalid calculated d_xi_dx = {} at x = {}",
                    d_xi_dx_calculated, x_interp)));
  }

  // Wall temperature (Hermite)
  if (wall_config.wall_temperatures.size() != edge_config.edge_points.size()) {
    return std::unexpected(BoundaryConditionError(std::format(
        "Wall temperatures size ({}) doesn't match edge points size ({})",
  ...
  if (!std::isfinite(d_ue_dxi) || !std::isfinite(edge.d_he_dxi)) {
    return std::unexpected(BoundaryConditionError(std::format(
        "Invalid computed derivatives: d_ue_dxi = {}, d_he_dxi = {}", d_ue_dxi,
        edge.d_he_dxi)));
  }
  ...
  return BoundaryConditions{
      .edge = std::move(edge),
      .wall = std::move(wall),
      .beta = compute_beta(station, xi, sim_config,
                           /*u_e=*/velocity_result.value(), d_ue_dxi),
      .station = station,
      .xi = xi,
      .sim_config = &sim_config};
}
