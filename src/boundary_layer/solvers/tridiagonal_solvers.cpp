...
[[nodiscard]] auto compute_collocation_coeffs(double a, double b, double c, double t)
  ...
  if (t == -1.0) {
    ...
    return std::unexpected(SolverError(std::format("Invalid collocation point t={}, expected -1, 0, or 1", t)));
  }
...
auto solve_species_block_tridiagonal(const core::Matrix<double>& prev_solution, std::span<const double> f_bc,
  ...
  for (auto& mat : A)
    mat = core::Matrix<double>(n_heavy, n_heavy);
  for (auto& mat : B)
    mat = core::Matrix<double>(n_heavy, n_heavy);
  for (auto& mat : C)
    mat = core::Matrix<double>(n_heavy, n_heavy);
  ...
  for (std::size_t i = 1; i < n_eta - 1; ++i) {
    for (std::size_t j = start_idx; j < n_species; ++j) {
      ...
      auto col_m1_result = compute_collocation_coeffs(a_coeffs(i - 1, j), b_coeffs(i - 1, j), c_coeffs(i - 1, j), -1.0);
      if (!col_m1_result) {
        return std::unexpected(col_m1_result.error());
      }
      auto col_m1 = col_m1_result.value();

      auto col_0_result = compute_collocation_coeffs(a_coeffs(i, j), b_coeffs(i, j), c_coeffs(i, j), 0.0);
      if (!col_0_result) {
        return std::unexpected(col_0_result.error());
      }
      auto col_0 = col_0_result.value();

      auto col_p1_result = compute_collocation_coeffs(a_coeffs(i + 1, j), b_coeffs(i + 1, j), c_coeffs(i + 1, j), 1.0);
      if (!col_p1_result) {
        return std::unexpected(col_p1_result.error());
      }
      auto col_p1 = col_p1_result.value();
      ...
      C[i](jh, jh) = col_m1.m1 * det_m1[jh] + col_0.m1 * det_0[jh] + col_p1.m1 * det_p1[jh];
      B[i](jh, jh) = col_m1.z * det_m1[jh] + col_0.z * det_0[jh] + col_p1.z * det_p1[jh];
      A[i](jh, jh) = col_m1.p1 * det_m1[jh] + col_0.p1 * det_0[jh] + col_p1.p1 * det_p1[jh];

      D(i, j) = d_rhs(i - 1, j) * det_m1[jh] + d_rhs(i, j) * det_0[jh] + d_rhs(i + 1, j) * det_p1[jh];
    }
  }
  ...
  for (std::size_t j = start_idx; j < n_species; ++j) {
    ...
    auto col_1_result = compute_collocation_coeffs(a_coeffs(0, j), b_coeffs(0, j), c_coeffs(0, j), -1.0);
    if (!col_1_result) {
      return std::unexpected(col_1_result.error());
    }
    col_1[jh] = col_1_result.value();

    auto col_2_result = compute_collocation_coeffs(a_coeffs(1, j), b_coeffs(1, j), c_coeffs(1, j), 0.0);
    if (!col_2_result) {
      return std::unexpected(col_2_result.error());
    }
    ...
    R0(jh, jh) = col_bc[jh].m1 * det_m1[jh] + col_1[jh].m1 * det_0[jh] + col_2[jh].m1 * det_p1[jh];
    R1(jh, jh) = col_bc[jh].z * det_m1[jh] + col_1[jh].z * det_0[jh] + col_2[jh].z * det_p1[jh];
    R2(jh, jh) = col_bc[jh].p1 * det_m1[jh] + col_1[jh].p1 * det_0[jh] + col_2[jh].p1 * det_p1[jh];
    R[jh] = h_bc[j] * det_m1[jh] + d_rhs(0, j) * det_0[jh] + d_rhs(1, j) * det_p1[jh];
  }
