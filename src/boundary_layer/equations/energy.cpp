...
auto solve_energy(std::span<const double> g_previous,
  ...
  auto energy_coeffs_result = detail::build_energy_coefficients(
      g_previous, inputs, coeffs, bc, xi_der, sim_config, F_field, dF_deta,
      V_field, mixture, station, d_eta);
  ...
  auto boundary_conds = detail::build_energy_boundary_conditions(
      coeffs, bc, sim_config, mixture, inputs, station, d_eta);

  // Solve tridiagonal system
  auto solution_result = solvers::solve_momentum_energy_tridiagonal(
      g_previous, boundary_conds.f_bc, boundary_conds.g_bc, boundary_conds.h_bc,
      energy_coeffs.a, energy_coeffs.b, energy_coeffs.c, energy_coeffs.d);
...
[[nodiscard]] auto
compute_dufour_term(const coefficients::CoefficientInputs &inputs,
                    const coefficients::CoefficientSet &coeffs,
                    const conditions::BoundaryConditions &bc,
                    const thermophysics::MixtureInterface &mixture,
                    std::size_t eta_index, PhysicalQuantity auto d_eta)
    -> std::expected<double, EquationError>;

auto build_energy_coefficients(std::span<const double> g_previous,
                               const coefficients::CoefficientInputs &inputs,
                               const coefficients::CoefficientSet &coeffs,
                               const conditions::BoundaryConditions &bc,
                               const coefficients::XiDerivatives &xi_der,
  ...
  const auto J_fact_result =
      compute_energy_j_factor(station, xi, bc, sim_config);
  if (!J_fact_result) {
  ...
  if (sim_config.consider_dufour_effect) {
    ...
    for (std::size_t i = 0; i < n_eta; ++i) {
      auto dufour_result =
          compute_dufour_term(inputs, coeffs, bc, mixture, i, d_eta);
      if (!dufour_result) {
    ...
    auto dufour_derivative_result =
        coefficients::derivatives::compute_eta_derivative(dufour_terms, d_eta);
    if (!dufour_derivative_result) {
      return std::unexpected(
          EquationError("Failed to compute Dufour term derivative"));
    }
  ...
  for (std::size_t i = 0; i < n_eta; ++i) {
    ...
    double he = bc.he();
    double c_term =
        -2.0 * xi_i * F_i * dhe_dxi / he - 2.0 * xi_i * F_i * lambda0;
    energy_coeffs.c.push_back(c_term);

    // Compute species enthalpy terms
    auto [tmp1, tmp2] =
        compute_species_enthalpy_terms(inputs, coeffs, bc, J_fact, i);
    ...
    const double d_term =
        ...
        2.0 * xi * F_field[i] * g_derivatives[i] + tmp1 * K_bl_sq +
        tmp2 * coeffs.transport.K_bl;
...
build_energy_boundary_conditions(const coefficients::CoefficientSet &coeffs,
  ...
  bool flux_bc = sim_config.adiabatic || sim_config.radiation;
  ...
  if (!flux_bc) {
    ...
    // Calcul des termes de flux diffusifs pour la condition au flux
    for (std::size_t i = 0; i < n_species; ++i) {
      ...
      boundary_conds.h_bc +=
          Pr_wall / l0_wall * h_sp_wall / bc.he() * J_fact * J_wall;

      // Terme 3: Effet Dufour si activé
      if (sim_config.consider_dufour_effect &&
          coeffs.thermal_diffusion.tdr.rows() > 0) {
        ...
        if (std::abs(c_wall) > 1e-15 && std::abs(rho_wall) > 1e-15) {
          boundary_conds.h_bc += bc.P_e() / bc.he() * Pr_wall / l0_wall *
                                 tdr_wall * J_fact * J_wall /
                                 (c_wall * rho_wall);
        }
      }
    }

    if (sim_config.radiation) {
      const double eps_w = bc.wall.emissivity;
      const double T_env = bc.wall.environment_temperature;
      const double q_rad = eps_w * thermophysics::constants::sigma *
                           (std::pow(bc.Tw(), 4) - std::pow(T_env, 4));

      const double C = coeffs.transport.K_bl;
      const double rho_mu_r = bc.rho_e() * bc.mu_e();
      const double sqrt_2xi = std::sqrt(2.0 * bc.xi);
      double r_pow = 1.0;
      if (sim_config.body_type != io::SimulationConfig::BodyType::TwoD &&
          sim_config.body_type != io::SimulationConfig::BodyType::FlatPlate) {
        r_pow = bc.r_body();
      }
      const double factor =
          -Pr_wall / (C * rho_mu_r) * sqrt_2xi / (bc.he() * bc.ue() * r_pow);
      boundary_conds.h_bc -= q_rad * factor;
    }
  }
...
compute_dufour_term(const coefficients::CoefficientInputs &inputs,
  ...
  auto mole_fractions_result =
      mixture.mass_fractions_to_mole_fractions(mass_fractions);
  if (!mole_fractions_result) {
    return std::unexpected(
        EquationError("Failed to compute mole fractions for Dufour term"));
  }
  ...
  if (!mixture_mw_result) {
    return std::unexpected(
        EquationError("Failed to compute mixture MW for Dufour term"));
  }
...
template auto solve_energy<double>(
    ...
    std::span<const double> dF_deta, std::span<const double> V_field,
    const thermophysics::MixtureInterface &mixture, int station, double d_eta)
    -> std::expected<std::vector<double>, EquationError>;
