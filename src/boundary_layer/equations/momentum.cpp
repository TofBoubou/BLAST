...
auto solve_momentum(std::span<const double> F_previous, const coefficients::CoefficientSet& coeffs,
                    const conditions::BoundaryConditions& bc, const coefficients::XiDerivatives& xi_der,
                    std::span<const double> V_field, PhysicalQuantity auto xi,
  ...
  if (n_eta != V_field.size()) {
    return std::unexpected(EquationError("Momentum: F_previous and V_field size mismatch: {} vs {}",
                                         std::source_location::current(), n_eta, V_field.size()));
  }

  if (n_eta != coeffs.transport.l0.size()) {
    return std::unexpected(EquationError("Momentum: incompatible coefficient sizes"));
  }

  // Build tridiagonal system coefficients
  auto momentum_coeffs = detail::build_momentum_coefficients(F_previous, coeffs, bc, xi_der, V_field, xi, d_eta);
  ...
  auto solution_result = solvers::solve_momentum_energy_tridiagonal(
      F_previous, boundary_conds.f_bc, boundary_conds.g_bc, boundary_conds.h_bc, momentum_coeffs.a, momentum_coeffs.b,
      momentum_coeffs.c, momentum_coeffs.d);

  if (!solution_result) {
    return std::unexpected(EquationError("Momentum: tridiagonal solver failed: {}", std::source_location::current(),
...
auto build_momentum_coefficients(std::span<const double> F_previous, const coefficients::CoefficientSet& coeffs,
                                 const conditions::BoundaryConditions& bc, const coefficients::XiDerivatives& xi_der,
                                 std::span<const double> V_field, PhysicalQuantity auto xi,
                                 PhysicalQuantity auto d_eta) noexcept -> MomentumCoefficients {
  ...
  const double T_edge =
      bc.P_e() * coeffs.thermodynamic.MW[n_eta - 1] / (coeffs.thermodynamic.rho[n_eta - 1] * R_universal);
  ...
  for (std::size_t i = 0; i < n_eta; ++i) {
    ...
    const double d_term_si_linearisation =
        -bc.beta * (rho_e_actual / coeffs.thermodynamic.rho[i]) + 2.0 * xi * F_previous[i] * F_derivatives[i];
  }
...
template auto solve_momentum<double, double>(std::span<const double> F_previous,
                                             const coefficients::CoefficientSet& coeffs,
                                             const conditions::BoundaryConditions& bc,
                                             const coefficients::XiDerivatives& xi_der, std::span<const double> V_field,
                                             double xi,
