...
auto ThermodynamicConsistencyStep::execute(SolverContext& ctx) -> std::expected<void, StepExecutionError> {
  ...
  if (!update_result) {
    return std::unexpected(
        StepExecutionError(name(), std::format("Edge properties update failed: {}", update_result.error().what())));
  }
...
auto MechanicalResolutionStep::execute(SolverContext& ctx) -> std::expected<void, StepExecutionError> {
  ...
  if (!V_result) {
    return std::unexpected(
        StepExecutionError(name(), std::format("Continuity equation failed: {}", V_result.error().message())));
  }
  ...
  if (!coeffs_result) {
    return std::unexpected(
        StepExecutionError(name(), std::format("Coefficient calculation failed: {}", coeffs_result.error().message())));
  }
...
auto ThermalResolutionStep::execute(SolverContext& ctx) -> std::expected<void, StepExecutionError> {
  ...
  auto thermal_inputs = coefficients::CoefficientInputs{.xi = ctx.xi,
                                                        .F = ctx.solution.F,
                                                        .c = ctx.solution.c,
...
auto ChemicalResolutionStep::execute(SolverContext& ctx) -> std::expected<void, StepExecutionError> {
  ...
  auto chemical_inputs = coefficients::CoefficientInputs{.xi = ctx.xi,
                                                         .F = ctx.solution.F,
                                                         .c = ctx.solution.c,
                                                         .dc_deta = derivatives.dc_deta,
                                                         .dc_deta2 = derivatives.dc_deta2,
  ...
  if (!c_result) {
    return std::unexpected(
        StepExecutionError(name(), std::format("Species equations failed: {}", c_result.error().message())));
  }
...
auto InputCalculationStep::execute(SolverContext& ctx) -> std::expected<void, StepExecutionError> {
  ...
  current_inputs_ = std::make_unique<coefficients::CoefficientInputs>(
      coefficients::CoefficientInputs{.xi = ctx.xi,
                                      .F = ctx.solution.F,
                                      .c = ctx.solution.c,
                                      .dc_deta = derivatives.dc_deta,
                                      .dc_deta2 = derivatives.dc_deta2,
...
auto SolverPipeline::create_for_solver(BoundaryLayerSolver& solver) -> SolverPipeline {
  ...
  pipeline.steps_.push_back(std::make_unique<ThermodynamicConsistencyStep>(solver.get_h2t_solver()));
  pipeline.steps_.push_back(
      std::make_unique<MechanicalResolutionStep>(solver.get_grid(), solver.get_coeff_calculator()));
  pipeline.steps_.push_back(std::make_unique<ThermalResolutionStep>());
  pipeline.steps_.push_back(std::make_unique<ChemicalResolutionStep>());
  pipeline.steps_.push_back(std::make_unique<BoundaryEnforcementStep>());
