...
Gasp2Catalysis::Gasp2Catalysis(const std::string& xml_file, const std::vector<std::string>& species_order, 
    ...
    : species_order_(species_order), molar_masses_(molar_masses), initialized_(false) {
  
  // Initialize GASP2
  auto init_result = gasp2::initialize_catalysis(species_order_, molar_masses_, xml_file);
  if (!init_result) {
