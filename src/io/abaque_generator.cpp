...
auto AbaqueGenerator::save_results(const Result& results, 
  ...
  // Save temperatures
  {
    ...
    hid_t dset_id = H5Dcreate2(file_id, "temperatures", H5T_NATIVE_DOUBLE, 
                               space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    
    if (dset_id >= 0) {
      H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, 
               results.temperatures.data());
      H5Dclose(dset_id);
    } else {
      success = false;
    }
  ...
  // Save catalyticity values
  {
    ...
    hid_t space_id = H5Screate_simple(1, dims, nullptr);
    hid_t dset_id = H5Dcreate2(file_id, "catalyticity_values", H5T_NATIVE_DOUBLE,
                               space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    
    if (dset_id >= 0) {
      H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               results.catalyticity_values.data());
      H5Dclose(dset_id);
    } else {
      success = false;
    }
  ...
  // Save heat flux matrix
  {
    ...
    hid_t dset_id = H5Dcreate2(file_id, "heat_fluxes", H5T_NATIVE_DOUBLE,
                               space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  // Add metadata as attributes
  {
    ...
    hsize_t dims[1] = {2};
    hid_t space_id = H5Screate_simple(1, dims, nullptr);
    hid_t attr_id = H5Acreate2(root_group, "temperature_range", H5T_NATIVE_DOUBLE,
    ...
    attr_id = H5Acreate2(root_group, "n_temperatures", H5T_NATIVE_INT,
    ...
    attr_id = H5Acreate2(root_group, "n_catalyticity_values", H5T_NATIVE_INT,
                         scalar_space, H5P_DEFAULT, H5P_DEFAULT);
    if (attr_id >= 0) {
