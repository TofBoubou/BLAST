...
auto HDF5Writer::write(
    ...
    try {
        if (progress) progress(0.0, "Creating HDF5 file");
        ...
        if (progress) progress(0.1, "Writing metadata");
        ...
        if (progress) progress(0.3, "Writing station data");
        ...
        return std::unexpected(OutputError(
            std::format("HDF5 write failed: {}", e.what())
        ));
    }
}

auto HDF5Writer::create_file(const std::filesystem::path& file_path) const 
    -> std::expected<FileHandle, OutputError> {
    ...
    if (file_id < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to create HDF5 file: {}", file_path.string())
        ));
    }
...
auto HDF5Writer::write_metadata(
    FileHandle& file,
    ...
    if (auto result = write_vector(mixture_group, "species_molecular_weights", metadata.mixture.species_molecular_weights); !result) {
        return std::unexpected(result.error());
    }
...
auto HDF5Writer::write_stations(
    FileHandle& file,
...
auto HDF5Writer::write_station_data(
    GroupHandle& station_group,
    ...
    if (auto result = write_scalar(station_group, "x_physical", station.x_physical, "m", "Physical x coordinate"); !result) {
        return std::unexpected(result.error());
    }
    if (auto result = write_vector(station_group, "eta", station.eta, "", "Similarity coordinate"); !result) {
        return std::unexpected(result.error());
    }
    if (auto result = write_vector(station_group, "y_physical", station.y_physical, "m", "Physical y coordinate"); !result) {
        return std::unexpected(result.error());
    }
...
auto HDF5Writer::create_group(
    hid_t parent,
    const std::string& name
) const -> std::expected<GroupHandle, OutputError> {
    
    auto group_id = H5Gcreate2(parent, name.c_str(), H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    if (group_id < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to create group '{}'", name)
        ));
    }
...
auto HDF5Writer::write_vector(
    ...
    if (space_id < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to create dataspace for '{}'", name)
        ));
    }
    ...
    auto dataset_id = H5Dcreate2(parent, name.c_str(), H5T_NATIVE_DOUBLE, space, 
                                H5P_DEFAULT, props, H5P_DEFAULT);
    if (dataset_id < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to create dataset '{}'", name)
        ));
    }
    ...
    if (status < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to write data for '{}'", name)
        ));
    }
...
auto HDF5Writer::write_matrix(
    hid_t parent,
    const std::string& name,
    const core::Matrix<double>& data,
    const std::string& units,
    const std::string& description
) const -> std::expected<void, OutputError> {
    ...
    if (space_id < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to create dataspace for matrix '{}'", name)
        ));
    }
    DataspaceHandle space(space_id);
    ...
    if (status < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to write matrix data for '{}'", name)
        ));
    }
...
auto HDF5Writer::write_scalar(
    ...
    if (space_id < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to create scalar dataspace for '{}'", name)
        ));
    }
    DataspaceHandle space(space_id);
    ...
    if (dataset_id < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to create scalar dataset '{}'", name)
        ));
    }
    DatasetHandle dataset(dataset_id);
    ...
    if (status < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to write scalar value for '{}'", name)
        ));
    }
...
auto HDF5Writer::write_string(
    ...
    if (space_id < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to create dataspace for string '{}'", name)
        ));
    }
    DataspaceHandle space(space_id);
    ...
    if (obj_type == H5I_DATASET || obj_type == H5I_GROUP) {
        // Write as attribute
        auto attr_id = H5Acreate2(parent, name.c_str(), string_type, space, 
                                 H5P_DEFAULT, H5P_DEFAULT);
        if (attr_id < 0) {
            return std::unexpected(OutputError(
                std::format("Failed to create string attribute '{}'", name)
            ));
        }
        ...
        if (status < 0) {
            return std::unexpected(OutputError(
                std::format("Failed to write string attribute '{}'", name)
            ));
        }
    }
...
auto HDF5Writer::write_string_array(
    hid_t parent,
    const std::string& name,
    ...
    if (space_id < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to create dataspace for string array '{}'", name)
        ));
    }
    DataspaceHandle space(space_id);
    
    // Create dataset
    auto dataset_id = H5Dcreate2(parent, name.c_str(), string_type, space, 
                                H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    if (dataset_id < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to create string array dataset '{}'", name)
        ));
    }
    DatasetHandle dataset(dataset_id);
    ...
    if (status < 0) {
        return std::unexpected(OutputError(
            std::format("Failed to write string array data for '{}'", name)
        ));
    }
...
auto HDF5Writer::create_dataset_properties() const 
    -> std::expected<PropertyHandle, OutputError> {
...
auto validate_file(const std::filesystem::path& file_path) 
    ...
    if (result <= 0) {
        return std::unexpected(OutputError(
            std::format("Not a valid HDF5 file: {}", file_path.string())
        ));
    }
