...
auto YamlParser::parse() const
  ...
  try {
    ...
    if (has_boundary_override) {
      if (config.simulation.chemical_mode !=
          SimulationConfig::ChemicalMode::NonEquilibrium) {
        std::cout << "INFO: Forcing chemical_mode to 'non_equilibrium' because "
                     "boundary_override is used"
                  << std::endl;
        config.simulation.chemical_mode =
            SimulationConfig::ChemicalMode::NonEquilibrium;
      }
    }
...
auto YamlParser::parse_simulation_config(const YAML::Node &node) const
  ...
  try {
    ...
    if (node["finite_thickness"]) {
      auto finite_thickness_result =
          extract_value<bool>(node, "finite_thickness");
    ...
    if (node["radiation"]) {
      auto radiation_result = extract_value<bool>(node, "radiation");
      if (!radiation_result) {
        return std::unexpected(radiation_result.error());
      }
      config.radiation = radiation_result.value();
    }
...
auto YamlParser::parse_mixture_config(const YAML::Node &node) const
  ...
  try {
    ...
    if (node["thermal_conductivity_algorithm"]) {
      auto thermal_result =
          extract_enum(node, "thermal_conductivity_algorithm",
                       enum_mappings::thermal_conductivity_algorithms);
      if (!thermal_result)
...
auto YamlParser::parse_outer_edge_config(const YAML::Node &node) const
  ...
  try {
        ...
        if (boundary_override_node.IsScalar()) {
          ...
          if (point_node["mass_fraction_condition"]) {
            if (point.boundary_override.enabled) {
              auto mass_fractions = point_node["mass_fraction_condition"]
                                        .as<std::vector<double>>();

              double sum = std::accumulate(mass_fractions.begin(),
                                           mass_fractions.end(), 0.0);
              if (std::abs(sum - 1.0) > 1e-6) {
                return std::unexpected(core::ConfigurationError(std::format(
                    "mass_fraction_condition must sum to 1.0 (current sum: {})",
                    sum)));
              }
          ...
          // New format: boundary_override: { enabled: true,
          // mass_fraction_condition: [...] }
          if (boundary_override_node["enabled"]) {
            point.boundary_override.enabled =
                boundary_override_node["enabled"].as<bool>();
          }

          if (point.boundary_override.enabled) {
            if (!boundary_override_node["mass_fraction_condition"]) {
              return std::unexpected(core::ConfigurationError(
                  "mass_fraction_condition is required when "
                  "boundary_override.enabled is true"));
            }

            auto mass_fractions =
                boundary_override_node["mass_fraction_condition"]
                    .as<std::vector<double>>();
    ...
    return std::unexpected(core::ConfigurationError(
        std::format("In 'outer_edge' section: YAML error - {}", e.what())));
  }
}
...
auto YamlParser::parse_wall_parameters_config(const YAML::Node &node) const
  ...
  try {
    ...
    if (node["emissivity"]) {
      auto eps_result = extract_value<double>(node, "emissivity");
      if (!eps_result)
        return std::unexpected(eps_result.error());
      config.emissivity = eps_result.value();
    }

    if (node["environment_temperature"]) {
      auto tenv_result = extract_value<double>(node, "environment_temperature");
      if (!tenv_result)
        return std::unexpected(tenv_result.error());
      config.environment_temperature = tenv_result.value();
    }
    return config;
