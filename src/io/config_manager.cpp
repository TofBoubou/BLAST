...
auto ConfigurationManager::resolve_config_path(std::string_view config_file) const 
    ...
    if (!std::filesystem::exists(path_candidate)) {
        return std::unexpected(
            core::FileError{"could not locate config file", std::string(config_file)}
        );
    }
...
auto ConfigurationManager::load(std::string_view config_file) 
    ...
    if (!path_result) {
        return std::unexpected(
            core::ConfigurationError(std::format("Failed to resolve config path: {}", path_result.error().message())) // .error() to go to the exception
        );
    }
    
    config_file_path_ = path_result.value();  // extract the value of std::expected
    
    parser_ = std::make_unique<YamlParser>(config_file_path_.string()); //free the memory automatically, use the contructor of YamlParser with the arguments after, type std::unique_ptr<YamlParser> 
    
    auto load_result = parser_->load(); // normally include nothing because it in an expected void, error. Fill the root
    if (!load_result) {
        return std::unexpected(
