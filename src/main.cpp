...
int main(int argc, char* argv[]) {
  ...
  // Check if abaque generation is enabled - if so, skip normal simulation
  if (config.abaque.enabled) {
    std::cout << "\n=== GENERATING ABAQUE ===" << std::endl;
    std::cout << "Abaque mode enabled - skipping normal simulation" << std::endl;
    std::cout << "Temperature range: " << config.abaque.temperature_min << " - " << config.abaque.temperature_max
              << " K" << std::endl;
