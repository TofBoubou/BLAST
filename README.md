...
```bash
git clone https://github.com/your-username/BLAST_MODERN.git
cd BLAST_MODERN
```
...
The project uses the LLVM coding style with specific adaptations:
...
1. **Compiler Optimization**: The default build uses `-O3 -march=native`
2. **Grid Resolution**: Balance `n_eta` and `eta_max` for accuracy vs speed
3. **Convergence**: Adjust `convergence_tolerance` based on required precision
4. **Chemical Models**: Simplified chemistry models reduce computational cost

### Typical Performance

- **Stagnation Point**: ~1-2 minutes (50 grid points)
- **Full Body**: ~5-15 minutes (multiple stations)
...
**MPP_DATA_DIRECTORY errors:**
The solver automatically sets the Mutation++ data directory. If issues persist:
```bash
export MPP_DATA_DIRECTORY=/path/to/BLAST_MODERN/libs/mutationpp/data
```

**Convergence problems:**
- Reduce `convergence_tolerance` temporarily
- Increase `max_iterations`
- Check mixture definition in Mutation++ database
- Verify boundary conditions in configuration

**Memory issues:**
- Reduce `n_eta` for large simulations
- Limit number of output stations
