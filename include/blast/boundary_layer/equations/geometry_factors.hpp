...
[[nodiscard]] auto compute_geometry_factors(
    int station,
    PhysicalQuantity auto xi,
    const conditions::BoundaryConditions& bc,
    const io::SimulationConfig& sim_config
) -> std::expected<GeometryFactors, EquationError> {
    ...
    if (station == 0) {
        ...
        switch (sim_config.body_type) {
            case io::SimulationConfig::BodyType::Axisymmetric: {
                ...
                if (d_ue_dx_val <= 0.0) {
                    return std::unexpected(EquationError(
                        std::format("Invalid d_ue_dx: {} (must be positive)", d_ue_dx_val)
                    ));
                }
            ...
            case io::SimulationConfig::BodyType::TwoD: {
                ...
                return GeometryFactors(
                    1.0 / sqrt_term,
                    1.0 / d_ue_dx_val,
                    std::sqrt(bc.rho_e() * bc.mu_e() / d_ue_dx_val)
                );
            }
        ...
        if (xi <= 0.0 || !std::isfinite(xi)) {
            return std::unexpected(EquationError(
                std::format("Invalid xi: {} (must be positive and finite)", xi)
            ));
        }
        ...
        return GeometryFactors(
            bc.r_body() * sqrt_2xi / bc.d_xi_dx(),
            2.0 * xi / (bc.ue() * bc.d_xi_dx()),
            sqrt_2xi / (bc.ue() * bc.r_body())
        );
    }
}

[[nodiscard]] auto compute_energy_j_factor(
    int station,
    PhysicalQuantity auto xi,
    const conditions::BoundaryConditions& bc,
    const io::SimulationConfig& sim_config
