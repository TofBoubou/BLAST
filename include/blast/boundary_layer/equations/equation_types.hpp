...
concept NumericRange = std::ranges::range<Range> && std::floating_point<std::ranges::range_value_t<Range>>;
...
struct GeometryFactors {
  ...
  constexpr GeometryFactors(double j, double w, double bc) noexcept : J_fact(j), W_fact(w), bc_fact(bc) {}
};

// Error type for equation solving operations
class EquationError : public core::BlastException {
public:
  explicit EquationError(std::string_view message, std::source_location location = std::source_location::current())
      : BlastException(std::format("Equation Error: {}", message), location) {}

  template <typename... Args>
  explicit EquationError(std::string_view format_str, std::source_location location, Args&&... args)
      : BlastException(std::format("Equation Error: {}", std::vformat(format_str, std::make_format_args(args...))),
                       location) {}
};
