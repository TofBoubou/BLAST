...
// Boundary conditions: specified temperature, adiabatic or radiative wall
[[nodiscard]] auto solve_energy(std::span<const double> g_previous,
...
[[nodiscard]] auto build_energy_coefficients(
...
[[nodiscard]] auto
compute_species_enthalpy_terms(const coefficients::CoefficientInputs &inputs,
                               const coefficients::CoefficientSet &coeffs,
                               const conditions::BoundaryConditions &bc,
                               double J_fact, std::size_t eta_index)
