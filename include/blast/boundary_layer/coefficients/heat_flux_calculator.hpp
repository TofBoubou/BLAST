...
private:
  ...
  [[nodiscard]] auto compute_local_conductivities(const CoefficientInputs& inputs,
  ...
  [[nodiscard]] auto
  compute_wall_heat_fluxes(const CoefficientInputs& inputs, const CoefficientSet& coeffs,
                           const conditions::BoundaryConditions& bc, int station,
                           double xi) const -> std::expected<std::tuple<double, double, double>, HeatFluxError>;

public:
  HeatFluxCalculator(const thermophysics::MixtureInterface& mixture, const io::SimulationConfig& sim_config,
  ...
  [[nodiscard]] auto calculate(const CoefficientInputs& inputs, const CoefficientSet& coeffs,
