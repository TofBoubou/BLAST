...
public:
  explicit XiDerivatives(std::size_t n_eta, std::size_t n_species) : n_eta_(n_eta), n_species_(n_species) {
    initialize();
  }
  ...
  void update_station(int station, double xi, std::span<const double> F, std::span<const double> g,
                      const core::Matrix<double>& c);

  // Handle convergence failure
  void update_failed_convergence(int station, double xi, std::span<double> F, std::span<double> g,
  ...
  [[nodiscard]] auto F_derivative() const noexcept -> std::span<const double> { return F_xi_der_; }
  [[nodiscard]] auto g_derivative() const noexcept -> std::span<const double> { return g_xi_der_; }
  [[nodiscard]] auto c_derivative() const noexcept -> const core::Matrix<double>& { return c_xi_der_; }
