...
template<typename HandleType, auto CloseFunc>
class HDF5Handle {
private:
    HandleType handle_;
...
public:
    ...
    // Move semantics only
    HDF5Handle(HDF5Handle&& other) noexcept : handle_(other.handle_) {
        other.handle_ = -1;
    }
...
private:
    ...
    [[nodiscard]] auto create_file(const std::filesystem::path& file_path) const 
    ...
    [[nodiscard]] auto write_station_data(
    ...
    [[nodiscard]] auto write_string(
        hid_t parent,
        const std::string& name,
        const std::string& value
    ) const -> std::expected<void, OutputError>;
    
    [[nodiscard]] auto write_string_array(
        hid_t parent,
        const std::string& name,
    ...
    [[nodiscard]] auto create_dataset_properties() const 
        -> std::expected<PropertyHandle, OutputError>;
    
    [[nodiscard]] auto create_chunked_properties(std::size_t size) const 
        -> std::expected<PropertyHandle, OutputError>;
...
public:
    ...
    // HDF5-specific configuration
    auto set_hdf5_config(HDF5Config config) noexcept -> void {
        hdf5_config_ = config;
    }
    
    [[nodiscard]] auto get_hdf5_config() const noexcept -> const HDF5Config& {
        return hdf5_config_;
    }
};
...
public:
    ...
    [[nodiscard]] auto read_dataset() const 
        -> std::expected<OutputDataset, OutputError>;
    
    // Read specific components
    [[nodiscard]] auto read_metadata() const 
        -> std::expected<SimulationMetadata, OutputError>;
    
    [[nodiscard]] auto read_station(int station_index) const 
        -> std::expected<StationData, OutputError>;
    
    [[nodiscard]] auto read_station_variable(
        int station_index,
        const std::string& variable_name
    ) const -> std::expected<std::vector<double>, OutputError>;
    
    [[nodiscard]] auto get_station_count() const 
        -> std::expected<int, OutputError>;
    
    [[nodiscard]] auto get_variable_names(int station_index = 0) const 
        -> std::expected<std::vector<std::string>, OutputError>;
    
    // Utility functions
    [[nodiscard]] auto is_valid_file(const std::filesystem::path& file_path) const noexcept 
        -> bool;
    
    [[nodiscard]] auto get_file_info() const 
        -> std::expected<std::map<std::string, std::string>, OutputError>;
};
...
namespace hdf5 {
    ...
    // Validate HDF5 file
    [[nodiscard]] auto validate_file(const std::filesystem::path& file_path) 
