...
#include "../io/abaque_generator.hpp"
...
public:
  struct SimulationResult {
    ...
    bool is_abaque = false;
    std::string output_filename;
  };

  // Run simulation (either standard or abaque)
...
private:
  ...
  // Run abaque generation
  [[nodiscard]] auto run_abaque_generation(
    boundary_layer::solver::BoundaryLayerSolver& solver,
    const thermophysics::MixtureInterface& mixture,
    const io::Configuration& config,
    const std::string& case_name,
  ...
  // Display abaque generation info
  auto display_abaque_info(const io::Configuration& config) const -> void;
};
